...
import edu.neu.coe.csye7200.asstwc.MonadOps.{asFuture, sequence}
...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[URL] =  for (n <- ns \\ "a"  map (_ \ "@href") map (_ toString) toList) yield new URL(u,n)// TO BE IMPLEMENTED

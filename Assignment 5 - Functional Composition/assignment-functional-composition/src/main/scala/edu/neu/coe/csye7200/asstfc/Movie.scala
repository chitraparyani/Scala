...
object Movie extends App {
  ...
  object MoviesProtocol extends DefaultJsonProtocol {
    // 20 points
    // TO BE IMPLEMENTED
    implicit val movFormat = jsonFormat4(Format.apply)
  ...
  def testSerializationAndDeserialization(ms: Seq[Movie]): Boolean = {
    ...
    val movieSer = ms.toJson
    def Serializ(str:String):Seq[Movie] = str.parseJson.convertTo[Seq[Movie]]
    val movieDeser = Serializ(movieSer.toString)
   // val movieDeser1 = movieSer.convertTo[Seq[Movie]].toString
    if(movieDeser == ms) true
    else false
...
//   val res = mov.toJson.convertTo.equals(ms)
//   mov == res
 //}

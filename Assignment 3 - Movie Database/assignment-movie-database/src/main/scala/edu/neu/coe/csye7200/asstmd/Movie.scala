...
/**
  ...
  * Although the limitation on 22 fields in a case class has partially gone away,
  * it's still convenient to group the different attributes together into logical classes.
  *
...
object Movie extends App {
  ...
  trait IngestibleMovie extends Ingestible[Movie] {
    ...
    def fromString(w: String): Try[Movie] = Try(Movie.apply(w.split(",").toList))

    // TO BE IMPLEMENTED 11 points
  }
...
object Rating {
  // Hint: This regex matches three patterns: (\w*), (-(\d\d)), (\d\d),
  // for example "PG-13", the first one matches "PG", second one "-13", third one "13".
  val rRating = """^(\w*)(-(\d\d))?$""".r
  ...
  def apply(s: String): Rating = s match {
    ...
    case rRating(null) => apply(null)
    case _ => throw new Exception(s"parse error in Name: $s")
